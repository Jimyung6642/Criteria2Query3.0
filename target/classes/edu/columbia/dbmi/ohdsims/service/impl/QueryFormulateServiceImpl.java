...
import static edu.columbia.dbmi.ohdsims.util.SQLUtil.*;
...
public class QueryFormulateServiceImpl implements IQueryFormulateService {
    ...
    public String getFullResult(Document doc) {
        ...
        if (doc.getInitial_event() != null){
            List<Paragraph> inclusion_ps = doc.getInclusion_criteria();
            sb.append("\n[Inclusion_criteria]\n");
            for (Paragraph p : inclusion_ps) {
                if (p.getSents() != null) {
                    for (Sentence s : p.getSents()) {
                        sb.append(s.getDisplay() + " ");
                    }
        ...
        if (doc.getExclusion_criteria() != null){
            List<Paragraph> exclusion_ps = doc.getExclusion_criteria();
            sb.append("\n[Exclusion_criteria]\n");
            for (Paragraph p: exclusion_ps) {
                if (p.getSents() != null) {
                    for (Sentence s : p.getSents()) {
                        sb.append(s.getDisplay() + " ");
                    }
        ...
        result = result.replaceAll("(<b>|</b>|<i>|</i>|</mark>|\\[|\\])", " $1 ");
        result = result.replaceAll("\\s+", " ");
        result = result.replace("\\sInitial_event\\s", "Initial_event");
        result = result.replace("\\sInclusion_criteria\\s", "Inclusion_criteria");
        result = result.replace("\\sExclusion_criteria\\s", "Exclusion_criteria");        
        return result;
    }
...
//                    criterialist.add(criterion);
    ...
    public String generateSQL(String fullResult, String explain) {
        String sql = generateSqlScript(fullResult, explain);
        return sql;
    }
